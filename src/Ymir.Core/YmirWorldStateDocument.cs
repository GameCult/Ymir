...
[MessagePackObject]
public sealed class YmirWorldStateDocument
{
    [Key(0)]
    [CultName]
    ...
    public static YmirWorldStateDocument FromSoA(string worldId, YmirSoAWorld world) => new()
    {
        WorldId = worldId,
        Time = world.Time,
...
public static class YmirCultCacheStore
{
    public static async Task SaveWorldAsync(string path, string worldId, YmirSoAWorld world)
    {
        ...
        await cache.UpsertAsync(
            YmirWorldStateDocument.FromSoA(worldId, world),
            new CultRecordHandle<YmirWorldStateDocument>(new CultRecordKey($"ymir:world:{worldId}")))
            .ConfigureAwait(false);
    ...
    public static async Task<YmirSoAWorld?> LoadWorldAsync(string path, string worldId)
    {
        ...
        return cache.Get<YmirWorldStateDocument>(new CultRecordKey($"ymir:world:{worldId}"))?.ToSoA();
    }
}
